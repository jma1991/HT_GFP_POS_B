...
```{r}
sce <- readRDS("data/02-normalization.rds")
```
...
hvg <- getTopHVGs(dec, var.field = "bio", var.threshold = 0, fdr.field = "p.value", fdr.threshold = 0.05)
...
hvg <- getTopHVGs(dec, var.field = "ratio", var.threshold = 1, fdr.field = "p.value", fdr.threshold = 0.05)
...
```{r}
saveRDS(sce, "data/03-feature-selection.rds")
```
